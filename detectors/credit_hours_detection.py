...
class CreditHoursDetector:
    ...
    def detect(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"field_name": self.field_name, "found": False, "content": "ERROR"}
    ...
    def _find_credits(self, text: str) -> tuple[bool, Optional[str]]:
        ...
        """
        # Search in the available text slice (detect() already caps large docs)
        # This improves recall for syllabi where credits appear deeper in the document.
        search_text = text
        ...
        for pattern in self.credit_patterns:
            for match in re.finditer(pattern, search_text, re.IGNORECASE):
                ...
                # Get context around the match. Use pre-context for prerequisite checks
                # to avoid skipping valid course-credit lines followed by "Prerequisites".
                before_start = max(0, match.start() - 80)
                before_context = search_text[before_start : match.start()].lower()
                after_end = min(len(search_text), match.end() + 80)
                after_context = search_text[match.end() : after_end].lower()
                ...
                if any(keyword in before_context for keyword in skip_keywords):
                ...
                if any(
                    keyword in before_context or keyword in after_context
                ...
                if number_match:
                    ...
                    if not (credit_number <= 12.0):
                        self.logger.debug(
                            f"Skipping unrealistic credit value: {full_match}"
                        )
                        continue
                ...
                # Canonicalize bare label format to improve cross-format matching.
                # Example: "Credits: 4" -> "Credits: 4 credits"
                m_label_only = re.match(
                    r"(?i)^credits?\s*:\s*(\d+(?:\.\d+)?)\s*$", credit_text
                )
                if m_label_only:
                    credit_text = f"Credits: {m_label_only.group(1)} credits"
                ...
                self.logger.debug(
                    f"Found potential credit: {credit_text} at position {position}"
                )
        ...
        # Fallback: handle forms like
        #   Credits:
        #   ... other labels ...
        #   4
        found, label_match = self._find_credit_from_label_block(search_text)
        if found:
            return True, label_match

        return False, None

    def _find_credit_from_label_block(self, text: str) -> tuple[bool, Optional[str]]:
        """
        Find credit values when the syllabus uses a label/value block where
        "Credits:" appears on one line and the numeric value appears several
        lines later in a profile table.
        """
        ...
        for i, raw_line in enumerate(lines):
            ...
            if not re.match(r"^credits?\s*:\s*$", line):
                continue

            # Scan forward a short window for a plausible numeric credit value.
            for j in range(i + 1, min(i + 15, len(lines))):
                candidate = lines[j].strip()
                if not candidate:
                    continue

                if candidate.lower().rstrip(":") in ignorable_labels:
                    continue

                m_num = re.match(r"^(\d+(?:\.\d+)?)\s*$", candidate)
                if not m_num:
                    continue

                number = float(m_num.group(1))
                if number > 12.0:
                    continue

                formatted = m_num.group(1)
                self.logger.info(
                    f"Found credit declaration from label block: Credits: {formatted} credits"
                )
                return True, f"Credits: {formatted} credits"
