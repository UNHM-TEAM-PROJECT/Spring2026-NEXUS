...
* When backend returns `preferred_contact_missing = True`, an amber chat card appears in the chat window automatically
* Card contains a text input, suggestion chips, and Submit button
* Uses a unique timestamp based ID to handle multiple uploads in one session

### Suggestion chips

* Three chips shown: Email, Phone, Office Hours
* Each chip checks data from `/upload` response, green if found, red if not found
* Green chip fills input with extracted syllabus value, red chip asks user to enter manually
* Teams and Canvas excluded, users type these freely in the input field

### Input Validation

* Email must end with `@unh.edu`
* Phone must be exactly 10 digits
* Office hours must contain at least one digit representing a time
* Input border turns red when invalid, green when valid

### Confirmation Step

* Before POST fires, user sees entered value in an amber badge with Confirm and Edit buttons
* Edit restores input with value pre-filled, Confirm fires the POST to `/submit_preferred_contact`
...
* Backend response is shown as a file preview inside the chat card before downloading
* User clicks Download to save or Copy to copy content to clipboard
* Toast notification appears briefly after copying

### Re-enter Value and Session History

* "Re-enter value" link lets user change value without reuploading
* Frontend remembers last submitted value during session and offers "Use again?" on next upload
