...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf_doc:
            ...
            logging.info(f"{file_name}: PDF has {total_pages} pages")
            ...
            for i, page in enumerate(pdf_doc.pages, 1):
                ...
                if page_text:
                    ...
                    try:
                        ...
                        chars = page.chars
                        if not chars:
                            logging.warning(f"{file_name}: Page {i}: No character objects found - likely image-based")
                        else:
                            logging.info(f"{file_name}: Page {i}: Found {len(chars)} character objects but extract_text() returned nothing")
                    except Exception as e:
                        logging.error(f"{file_name}: Page {i}: Error checking character objects: {e}")

                # Extract tables with more detailed logging
                tables = page.extract_tables()
                if tables:
                    logging.info(f"{file_name}: Page {i}: Found {len(tables)} table(s)")
                    for table_idx, table in enumerate(tables):
                        if table:
                            ...
                            if table_text:
                                ...
                                logging.info(f"{file_name}: Page {i}, Table {table_idx + 1}: Extracted {len(table_content)} characters")

    except Exception as e:
        logging.error(f"{file_name}: Error extracting PDF {pdf_path}: {e}")
        return None

    combined_text = "\n".join(text)

    if not combined_text.strip():
        logging.warning(f"{file_name}: No text extracted from {pdf_path}")
        return None
    else:
        logging.info(f"{file_name}: Extracted {len(combined_text)} characters from {total_pages} pages in {pdf_path}")
        # Calculate average per page for comparison
        avg_per_page = len(combined_text) / total_pages if total_pages > 0 else 0
        logging.info(f"{file_name}: Average {avg_per_page:.0f} characters per page")
        ...
        if avg_per_page < 1000:  # Less than ~1000 chars per page might indicate issues
            logging.warning(f"{file_name}: Low character count per page ({avg_per_page:.0f}) - possible scanned/image-based PDF")
            logging.info(f"{file_name}: Attempting alternative extraction methods...")
            ...
            if alternative_text and len(alternative_text) > len(combined_text):
                logging.info(f"{file_name}: Alternative extraction yielded {len(alternative_text)} characters (vs {len(combined_text)})")
                ...
                for page_num, page in enumerate(pdf_reader.pages, 1):
                    try:
                        ...
                        if page_text:
                            ...
                            logging.info(f"{file_name}: PyPDF2 - Page {page_num}: {len(page_text)} characters")
                        else:
                            logging.warning(f"{file_name}: PyPDF2 - Page {page_num}: No text extracted")
                    except Exception as e:
                        logging.error(f"{file_name}: PyPDF2 - Page {page_num} error: {e}")

                if pages_text:
                    alternative_text = "\n".join(pages_text)
                    logging.info(f"{file_name}: PyPDF2 total: {len(alternative_text)} characters")

        except Exception as e:
            logging.error(f"{file_name}: PyPDF2 extraction failed: {e}")
    ...
    if (not alternative_text or len(alternative_text) < 5000) and PYMUPDF_AVAILABLE:
        try:
            ...
            for page_num in range(doc.page_count):
                ...
                if page_text:
                    pages_text.append(page_text)
                    logging.info(f"{file_name}: PyMuPDF - Page {page_num + 1}: {len(page_text)} characters")
                else:
                    logging.warning(f"{file_name}: PyMuPDF - Page {page_num + 1}: No text extracted")

            doc.close()

            if pages_text:
                pymupdf_text = "\n".join(pages_text)
                logging.info(f"{file_name}: PyMuPDF total: {len(pymupdf_text)} characters")

                if not alternative_text or len(pymupdf_text) > len(alternative_text):
                    alternative_text = pymupdf_text

        except Exception as e:
            logging.error(f"{file_name}: PyMuPDF extraction failed: {e}")
...
def extract_text_from_docx(docx_path):
    ...
    try:
        ...
        logging.info(f"{file_name}: Extracted {paragraph_count} paragraphs")
        ...
        for table_idx, table in enumerate(doc.tables):
            ...
            if table_text:
                ...
                table_count += 1
                logging.info(f"{file_name}: Table {table_idx + 1}: Extracted {len(table_text)} rows, {sum(len(row) for row in table_text)} characters")

        if table_count > 0:
            logging.info(f"{file_name}: Extracted {table_count} tables total")
            ...
            try:
                if section.header.paragraphs:
                    ...
                    if header_text:
                        ...
                        logging.info(f"{file_name}: Section {section_idx + 1} header: {len(header_text)} characters")
                ...
                if section.footer.paragraphs:
                    ...
                    if footer_text:
                        ...
                        header_footer_count += 1
                        logging.info(f"{file_name}: Section {section_idx + 1} footer: {len(footer_text)} characters")
            except Exception as e:
